...
impl Component for MainModel {
    ...
    async fn init(mut init: Self::Init<'_>, sender: &ComponentSender<Self>) -> Result<Self, Error> {
        ...
        let mut terminal_label = Child::<Label>::init(&window).await?;
        terminal_label.set_text(t!("quick.terminal.command"));
        let mut terminal = Child::<Edit>::init(&window).await?;

        let mut input_mode_label = Child::<Label>::init(&window).await?;
        input_mode_label.set_text(t!("quick.input_mode"));
        let mut input_mode = Child::<ComboBox>::init(&window).await?;
        ...
        HANDLER.with_app(|a| {
            ...
            search_edit_alt.set_selection(match config.search_edit.alt {
                AltKind::None => 0,
                AltKind::Alt09 => 1,
                AltKind::Alt09AZ => 2,
            });

            result_list_alt.set_selection(match config.result_list.alt {
                AltKind::None => 0,
                AltKind::Alt09 => 1,
                AltKind::Alt09AZ => 2,
            });
            ...
            terminal_kind.set_selection(match config.result_list.terminal_kind {
                TerminalKind::None => 0,
            ...
            input_mode.set_selection(match config.input_mode {
                InputMode::Auto => 0,
                InputMode::WmKey => 1,
